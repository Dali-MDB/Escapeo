    ...
    <div className="w-screen h-screen text-black bg-[#EEDAC4] flex justify-center items-center gap-10">
      ...
      <div className="w-3/6 h-[80%] flex flex-col gap-10 justify-start pt-10 items-center">
        ...
        <div className="w-3/4 flex flex-col justify-center items-start gap-4">
          <h1 className="text-black text-4xl font-bold">Set a password</h1>
        ...
        <div className="w-full grid grid-cols-1 grid-rows-2 gap-4">
            {[
              { type: "password", name: "Create Password" },
              { type: "password", name: "Re-enter Password" },
            ].map((el, index) => (
              <InputLogin key={index} type={el.type} name={el.name} />
            ))}
          </div> <button type="submit" className="w-full font-bold py-3 rounded-md bg-[#ED881F] text-black">
            Set password
          </button>
        </form>
