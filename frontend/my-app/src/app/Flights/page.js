    ...
    <div className="w-full mt-36 mx-auto flex flex-col items-center gap-4">
      ...
      <div className="flex w-[80%] mx-auto justify-center gap-2 flex-row ">
        {filters.slice(0, 8).map((filter, index) => (
          <button
            key={index}
            className={`px-4 w-full py-2 bg-white rounded-full border-4 shadow-[2px_-3px_10px_rgba(0,0,0,0.4)] ${filter.borderColor}`}
          >
            {filter.label}
          </button>
        ))}
      ...
      <div className="flex justify-center mx-auto gap-2 flex-row w-[90%]">
        {filters.slice(8, 17).map((filter, index) => (
          <button
            key={index}
            className={`px-4 w-full py-2 bg-white rounded-full border-4 shadow-[2px_-3px_10px_rgba(0,0,0,0.4)]
 ${filter.borderColor}`}
          >
            {filter.label}
          </button>
...
const FlightsSection = () => {
  return (
    <div className="w-[90%] mx-auto py-10 mt-20 gap-10 grid grid-cols-6 justify-center align-middle place-items-center">
      {BigOffers.map((BigOffer, index) => (
        <FlightCard key={index} {...BigOffer} />
      ))}
    </div>
  );
};
...
const Flights = () => (
  <div className="bg-[#EEDAC4] py-5">
    ...
    <div className="w-[90%] mx-auto py-10 tetx-center flex justify-center items-center">
      <button className="text-center py-4 w-2/3  text-xl text-white bg-[#235784] rounded-xl ">
        Show More
      </button>
    </div>
  </div>
