...
export default function Account() {
  ...
  useEffect(() => {
    if (profileData?.profile_picture) {
      setProfileImageUrl(`${API_URL}${profileData.profile_picture}?v=${imageVersion}`);
    } else {
      setProfileImageUrl(`${API_URL}${defaultProfilePic}?v=${imageVersion}`);
    }
  }, [profileData, imageVersion]);
  ...
  const handleProfilePictureChange = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProfileImageUrl(`${API_URL}${profileData?.profile_picture || defaultProfilePic}?v=${imageVersion}`);
          ...
          <div className="w-full h-full rounded-full relative overflow-hidden">
            <div className="relative w-full overflow-hidden rounded-full flex justify-center items-center">
              <Image
                ...
                unoptimized
                onError={(e) => {
                  e.target.src = `${API_URL}${defaultProfilePic}`;
                }}
                priority
              />
            </div>
        ...
        {isEditing ? (
          ...
          <div className="">
            {[
              ...
              {
                name: "Department",
                value: profileData?.department ? `${profileData.department || '' }`.trim()
                  : "N/A"
              },
            ].map((el, index) => (
              <div
