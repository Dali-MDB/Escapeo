...
export default function Messages() {
    ...
    const submit = async (e) => {
        ...
        const response = await fetch("http://localhost:3000/api/messages", {
            ...
            body: JSON.stringify({ username: 'John', message })
        })

        setMessages([...messages, { username: 'John', message }])
        setMessage("")
        setTimeout(() => {
            setLoading(false)
        }, 1000);
                    ...
                    <div className="px-3 py-2 border-t dark:border-zinc-700">
                        <div className="flex gap-2">
                            ...
                            <button type='submit' onClick={() => {
                                setMessages([...messages, { username: 'John', message }])
                                setMessage("")
                            }} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1.5 px-3 rounded-lg transition duration-300 ease-in-out text-sm" id="sendButton">
                                Send
                            </button>
                        </div>
                    </div>
