...
def TripsFiltering(request):
    ...
    # Apply filters
    if destination:
        query &= Q(destination__icontains=destination)  # Changed to use the correct field name
    ...
    if departure_date:
        try:
            ...
            query &= Q(departure_date__date=parsed_date)
        except ValueError:
    ...
    # Apply price filtering (assuming price is handled through DepartureTrip)
    if min_price or max_price:
        price_query = Q()
        if min_price:
            price_query &= Q(price__gte=min_price)
        if max_price:
            price_query &= Q(price__lte=max_price)
        
        # Get trips with matching prices
        matching_trip_ids = DepartureTrip.objects.filter(price_query).values_list('trip_id', flat=True)
        query &= Q(id__in=matching_trip_ids)
    ...
    if sort == "recommended":
        ...
        ).order_by('discount_price' if ascending else '-discount_price')
    elif sort == "bestseller":
        trips = trips.annotate(
            sold_percentage=ExpressionWrapper(
                F('sold_tickets') * 100.0 / F('capacity'),
                output_field=FloatField()
            )
        ).order_by('sold_percentage' if ascending else '-sold_percentage')
    else:
        # Default sorting
        valid_sort_fields = ["departure_date", "stars_rating", "discount"]
        if sort in valid_sort_fields:
            sort_param = sort if ascending else f"-{sort}"
            trips = trips.order_by(sort_param)
        else:
